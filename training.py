...
def np_load_with_pickle(npy_file):
    np_load_old = np.load
...
def train(input_dir, npy_file):
    nb_classes = len([name for name in os.listdir(
        input_dir) if name != ".DS_Store"])
    x_train, x_test, y_train, y_test = np_load_with_pickle(npy_file)
    ...
    checkpoint_cb = ModelCheckpoint(
        "snapshot/cat-bestmodel.hdf5", save_best_only=True)
